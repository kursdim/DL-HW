    ...
    * Если return_history=False
    ...
    for epoch in range(epochs):
        ...
        for vecs, labels in progress_bar(dataloader):
            ...
            loss = hinge_loss(model(vecs), labels)
            train_loss += loss.loss * len(labels)
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
            size += len(labels)
        ...
        if (epoch + 1) % step == 0 or (epoch + 1) == epochs:
            ...
            model.eval()
            dataloader = Dataloader(dataset, batch_size=len(dataset), is_train=False)

            train_acc = 0
            size = 0
            for vecs, labels in progress_bar(dataloader, text='Evaluating train'):
            ...
            if valid_dataset:
                ...
                for vecs, labels in progress_bar(valid_dataloader,
                    ...
                    # TODO: Реализовать рассчет accuracy модели на батче данных
                    preds = model(vecs)
                    valid_loss += hinge_loss(preds, labels).loss * len(labels)
